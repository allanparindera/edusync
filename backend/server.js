...
const authMiddleware = (req, res, next) => {
  ...
  try {
    ...
    if (secret) {
      ...
      // Insecure fallback for local development if secret is not set
...
// Apply JWT auth and rate limiter to the /api-proxy route
app.use('/api-proxy', authMiddleware, proxyLimiter);
...
app.post('/api-proxy', async (req, res) => {
  ...
  const cacheKey = JSON.stringify({ url: originalUrl, body: body });
  if (!apiClient.isStreaming) {
    const cachedResponse = aiCache.get(cacheKey);
    if (cachedResponse) {
      console.log(`[Node Proxy] Serving response from Cache for ${apiClient.name}`);
...
server.on('upgrade', async (request, socket, head) => {
  ...
  if (url.pathname === '/ws-proxy') {
    ...
    if (!token) {
      console.log('[Node Proxy] Missing WS token');
      socket.destroy();
      return;
    }
    ...
    try {
      ...
      console.log('[Node Proxy] Invalid WS token');
      socket.destroy();
      return;
    }
