...
(function() {
  ...
  window.WebSocket = function(url, protocols) {
    ...
    if (inputUrl && isValidUrl(inputUrl)) {
      ...
      const jwtToken = getCookie('edusync_supabase_jwt');
      const tokenParam = jwtToken ? `&token=${jwtToken}` : '';
      
      const proxyUrl = `${protocol}//${host}/ws-proxy?target=${targetUrl}${tokenParam}`;
      ...
      try {
        ...
        const proxyFetchOptions = {
          ...
          headers: {
            ...
            'X-App-Proxy': 'b34lYne3GSimuxD3fZAEaw3ITVIYHSnA',
